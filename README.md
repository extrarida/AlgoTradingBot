...
All trades, signals, account snapshots, and risk events are **persistently stored in a SQLite database**, so history is retained across sessions. The application also runs fully in **Demo Mode** on any machine without a real MT5 connection, making it easy to demonstrate and test on any operating system.
